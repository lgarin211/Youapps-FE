...
export default function ProfileImage({ selectedImage, userData, onEditClick }: ProfileImageProps) {
  return (
    <div className="mb-8">
      <div className="relative h-64 overflow-hidden bg-center bg-cover rounded-lg" style={{
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
            ...
            <div className="flex items-end space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-semibold text-white">
                  @{userData.username || 'user'}, {calculateAge(userData.birthday)}
                </h1>
                <p className="text-sm text-white/80">{userData.gender || 'Male'}</p>
              </div>
            </div>
          ...
          <div className="flex mt-4 space-x-3">
            <div className="flex items-center bg-black/30 backdrop-blur-sm rounded-full px-3 py-1.5">
              <span className="text-sm text-white">{userData.horoscope}</span>
            </div>
            <div className="flex items-center bg-black/30 backdrop-blur-sm rounded-full px-3 py-1.5">
              <span className="text-sm text-white">{userData.zodiac}</span>
            </div>
          </div>
        </div>
      </div>
